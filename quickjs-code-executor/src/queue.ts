 ...
 *
 * Guest code runs as one synchronous WASM call, so a CPU-bound script pins this
 * process's event loop until its deadline. Letting every inbound request start
 * immediately would turn one slow script into a service-wide stall, so runs are
